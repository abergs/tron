...
cycle = {
    ...
    checkCollision: function (cycle, opponent) {
        if ((cycle.x < (cycle.width / 2)) ||
            ...
            coordsUsed.indexOf(this.generateCoords(cycle)) >= 0) {
            return true;
        }
    },
...
loop = function () {
    if (game.started === true && game.over === false) {
...
chat.client.receive = function (playerId, direction) {
    console.log("receiving",direction);
    var playa  = game.getPlayer(playerId);
    enemy.current_direction = direction;
};

function sendDirection(direction) {
    console.log("sending", direction);
    chat.server.send(direction);
}